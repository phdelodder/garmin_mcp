        ...
        while current <= end:
            ...
            try:
                ...
                if data:
                    ...
                    # Some accounts still return the status fields wrapped in a
                    # trainingStatusDTO sub-object, and cycling-first athletes
                    # get the phrase under a cycling-specific key. Check every
                    # shape before giving up.
                    ts_dto = status_data.get("trainingStatusDTO") or {}
                    ts_phrase = (
                        status_data.get("trainingStatusFeedbackPhrase")
                        or status_data.get("trainingStatusCyclingFeedbackPhrase")
                        or ts_dto.get("trainingStatusFeedbackPhrase")
                        or ts_dto.get("trainingStatusCyclingFeedbackPhrase")
                    )
                    if ts_phrase:
